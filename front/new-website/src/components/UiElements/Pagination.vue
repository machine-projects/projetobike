...
const totalPages = ref<number>(Math.ceil(props.totalItems / props.perPage))

onUpdated(() => {
  totalPages.value = Math.ceil(props.totalItems / props.perPage)
})
...
<template>
  <div :class="{ 'flex justify-between items-center': goButton }">
    <div :class="{ 'flex justify-between items-center': (props.styled === 'simple') }">
      <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination"
        ...
        <a href="#" aria-current="page"
          ...
          @click.prevent="changePage(1)" v-if="hasFirst()">1</a>
        <span
        ...
        <a href="#" aria-current="page"
          class="bg-white border border-gray-300 text-gray-500 relative inline-flex items-center px-4 py-2 text-sm font-medium"
          v-if="hasLast()" @click.prevent="changePage(totalPages)">{{ totalPages }}</a>
        <a href="#"
    ...
    <div v-if="goButton">
      <label for="search" class="pr-3 text-gray-500 text-xs font-semibold uppercase">{{ textBeforeInput }}</label>
      <input type="text" placeholder="Page no"
        ...
        :class="`focus:${borderActiveColor}`" v-model="pageInput" />
      <button class="px-4 py-1.5 rounded-r bg-indigo-500 text-white" @click.prevent="changePage(pageInput)">{{
